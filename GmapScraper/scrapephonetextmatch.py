...
async def scrape_phone_number(address, denominationUniteLegale):
    async with async_playwright() as p:
        ...
        if title_element:
            ...
            if match < 0.5:
                ...
                print("Match found. Continuing to find phone number.")
        else:
...
async def main(start_row=0):
    ...
    for index, row in df[start_row:].iterrows():
        ...
        try:
            phone_number = await scrape_phone_number(address, row['denominationUniteLegale'])
